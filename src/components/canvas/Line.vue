...
export default {
  ...
  methods: {
    calculateLinePath (line) {
      // https://i.imgur.com/ab4ywFf.png (示意圖)
      var d = 'M ' + line[0].x + ' ' + line[0].y
      var minShrinkLength = this.calculateVectorLen(line[0], line[1])
      ...
      for (var i = 1; i < line.length - 1; i++) {
        ...
        if (line[i].x !== line[i + 1].x || line[i].y !== line[i + 1].y) {
          ...
          a = Math.min(this.calculateVectorLen(three, two), this.calculateVectorLen(one, two), a)

          // Calculation sweep_flag && ang(two line between)
          var sweepFlag = 0
          var ang = this.calculateUnitVector(two, three).ang - this.calculateUnitVector(two, one).ang
          if (ang > 0) {
            sweepFlag = 1
          } else {
            sweepFlag = 0
          ...
          var x4 = two.x - b * this.calculateUnitVector(two, one).x
          var y4 = two.y - b * this.calculateUnitVector(two, one).y
          var x5 = two.x - b * this.calculateUnitVector(two, three).x
          var y5 = two.y - b * this.calculateUnitVector(two, three).y

          // Ready for next time
          minShrinkLength = this.calculateVectorLen({ x: x5, y: y5 }, three)
    ...
    calculateUnitVector (a, b) {
      ...
      var unit = this.calculateVectorLen(a, b)
      var X = (a.x - b.x) / unit
      var Y = (a.y - b.y) / unit
      var ang = 0
      if (X > 0) {
